...
import Icon from 'react-native-vector-icons/Ionicons'; 
  ...
  const TreeMonthMealPlan = () => {
    ...
    useEffect(() => {
      axios
        .get(`http://192.168.1.102:8086/tree/one/${itemId.nutritionPlanId}`)
        .then((res) => {
          ...
          <View>
            <ImageBackground source={bgImage} style={style.Image}>
              ...
              <Icon name="ios-arrow-back" size={24} color="black" />
                ...
                <Text style={style.HeaderText}>{plan.treeMonthPlanName}</Text>
              </View>
              <View
            ...
            <ScrollView>
              ...
              <View>
                {plan && plan.springFive && plan.springFive.length > 0 && foods.map((food, index) => (
                  <View key={index} >
                    {plan.springFive[1] == food._id || plan.springFive[0] == food._id ? (
                      <Text style={style.nutriFood}>{food.foodName}</Text>
                    ) : null}
                  </View>
                ))}
              ...
              <View>
                {plan && plan.springSix && plan.springSix.length > 0 && foods.map((food, index) => (
                  <View key={index} >
                    {plan.springSix[1] == food._id || plan.springSix[0] == food._id ? (
                      <Text style={style.nutriFood}>{food.foodName}</Text>
                    ) : null}
                  </View>
                ))}
  ...
  export default TreeMonthMealPlan;
